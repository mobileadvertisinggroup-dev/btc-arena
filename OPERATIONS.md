...
| `/pilot-24h-archive/` | Created later by the reset script; preserves the 24-hour pilot results. |
...
## Exact activation command — 24-hour visible paper-trading pilot (NOT YET AUTHORIZED)

```
ARENA_PILOT_APPROVED=YES-AUDIT-PASSED ANTHROPIC_API_KEY=<key> \
  python3 scripts/run_pilot_24h.py --activate
```
...
dashboard with the banner: "24-HOUR PILOT — REAL AI DECISIONS — PAPER MONEY —
...
Preserves the pilot (page + raw data) under `/pilot-24h-archive/`, then writes
...
Claude model calls · hourly scheduler · the 24-hour pilot itself · official
